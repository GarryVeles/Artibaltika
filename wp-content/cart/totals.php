...
?>
<div style = 'width: auto; ' class="cart_totals <?php if ( $woocommerce->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
	...
	<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
		...
		<!--<h2 style = 'margin-bottom: 10px;'><?php _e( 'Cart Totals', 'woocommerce' ); ?></h2>-->
		...
		<table cellspacing="0">
			...
			<tr class="cart-subtotal" style="display:none;">
				...
				<?php if ( $woocommerce->cart->needs_shipping() && $woocommerce->cart->show_shipping() && ( $available_methods || get_option( 'woocommerce_enable_shipping_calc' ) == 'yes' ) ) : ?>
					...
					<tr class="shipping" style = 'position: absolute; top: -54px; left: 595px; font-weight: bold;'>
						<th><strong><?php _e( '', 'woocommerce' ); ?></strong></th>
						<td><strong><?php woocommerce_get_template( 'cart/shipping-methods.php', array( 'available_methods' => $available_methods ) ); ?></strong></td>
					</tr>
				...
				<tr class="total">
					<th><strong><?php _e( 'Итого:', 'woocommerce' ); ?></strong></th>
					<td>
