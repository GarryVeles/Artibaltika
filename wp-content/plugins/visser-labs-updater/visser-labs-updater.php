...
Description: Keep your Visser Labs plugins up to date.
Version: 1.0
Author: Visser Labs
...
function vl_updater_check_for_plugin_update( $checked_data ) {	
	...
	if ( is_wp_error( $plugin_list ) ) return;
	
	foreach ( $plugin_list as $plugin_slug => $plugin ) {

		if ( ! isset( $plugin['plugin_basename'] ) ) continue;
		...
		if ( file_exists( WP_PLUGIN_DIR . '/' . $plugin_basename ) && isset( $checked_data->checked[$plugin_basename] ) ) {
		    ...
		    if ( version_compare( $checked_data->checked[$plugin_basename], $plugin['version'], '<' ) ) {
		    	...
		    	$plugin_object->version     = $plugin['version'];
		    	$plugin_object->new_version = $plugin['version'];
		    	$plugin_object->slug        = $plugin_slug;
		    	$plugin_object->date        = $plugin['date'];
		    	$plugin_object->package     = $plugin['package'];
		    	$plugin_object->file_name   = $plugin['file_name'];
		    	$plugin_object->author      = $plugin['author'];
		    	$plugin_object->url         = $plugin['url'];
		    	$plugin_object->requires    = $plugin['requires'];
		    	$plugin_object->tested      = $plugin['tested'];
				$checked_data->response[$plugin_basename] = $plugin_object;
		    }
		}
	}
	return $checked_data;
