	...
	class WooCommerce_Delivery_Notes_Print {
		...
		public function generate_print_content_ajax() {		
			...
			if( empty( $_GET['template_type'] ) || empty( $_GET['order_id'] ) ) {
				...
				if( isset( $_GET['torg12']) ) {
				
					$order = new WC_Order( $_GET['order_id'] );
$items = $order->get_items();
//var_dump($items );?><table><?
foreach ( $items as $item ) {
    $product_name = $item['name'];
    $product_id = $item['product_id'];
 $product_sum = $item['line_total'] /$item['qty'];
...
<?
			...
			if( isset( $_GET['pdf']) ) {
				...
				header("Content-Disposition: attachment; filename=\"" . $this->order_id. ".html\";" );
