	...
	<table class="bar_chart">
		<thead>
			<tr>
				...
				<th ><?php _e( 'РАЗМЕР', 'woocommerce' ); ?></th>
				<th ><?php _e( 'ТИП', 'woocommerce' ); ?></th>
				<th >Продано</th>
				<th ><?php _e( 'ОСТАТОК', 'woocommerce' ); ?></th>
				<th width=15%>Цена</th>
...
$rrp = get_post_meta( $product_id, 'size', true );$rrp6 = get_post_meta( $product_id, '_regular_price', true );$rrp1 = get_post_meta( $product_id, 'tip', true );$rrp2 = get_post_meta( $product_id, '_stock', true );$rrp4 = get_post_meta( $product_id, 'tcs', true );$rrp7 = get_post_meta( $product_id, 'zakup', true );
...
$rrp11 = get_post_meta( $product_id, 'analog', true );$rrp12 = get_post_meta( $product_id, 'baltika', true );

//$rrp4=$product_title;
echo '<tr><td>'.get_post_meta( $product_id, 'tcs', true ).'</td>';
				//echo '<tr><td>'.$product_title .'</td>';
					echo '<td>'.$rrp.'</td>';		echo '<td>'.$rrp1.'</td>';
					...
					echo '<td>'.$rrp9.'</td>';
					echo '<td>'.$rrp10.'</td>';
						echo '<td>'.$rrp11.'</td>';	echo '<td>'.$rrp12.'</td>';
					echo '</tr>';
...
$y=0;
if ($a==1) $bb[]=array($rrp4,$rrp,$rrp1,(esc_html( $sales )),$rrp2,$rrp6,$rrp7,$rrp8,$rrp9,$rrp10,$rrp11,$rrp12);
else {
	for ($x=0;$x<count($bb);$x++)
	{
		if (trim($rrp4)==$bb[$x][0]) {$y=1; $bb[$x][3]=$bb[$x][3]+$tt;$bb[$x][4]=$bb[$x][4]+$rrp2;$bb[$x][13]=$bb[$x][13].','.$product_title;$bb[$x][12]=$bb[$x][12].'+'.$sales ;}
	}
	
	if ($y<>1) $bb[]=array($rrp4,$rrp,$rrp1,(esc_html( $sales )),$rrp2,$rrp6,$rrp7,$rrp8,$rrp9,$rrp10,$rrp11,$rrp12);
	}
$a++;
...
function woocommerce_stock_overview() {
	...
	$low_stock_products = (array) get_posts($params);
	
	
//	var_dump($low_stock_products);
	
	//print $lowstockamount;
