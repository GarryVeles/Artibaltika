	...
	switch ( $column ) {
		...
		case "order_title" :
			...
			if ( ! empty( $user_info ) ) {
           	...
           	echo '<a href="' . admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) . '"><strong>' . sprintf( __( 'Order %s', 'woocommerce' ), esc_attr( str_replace("#", "№", $the_order->get_order_number() ) ) ) . '</strong></a> ' . __( 'made by', 'woocommerce' ) . ' ' . $user;
