...
hipping3 = function (event) {	
 console.log($("#city2").val());
	ss=$("#city2").val();
	cc= $("#country option:selected").text();
	rr= $("#region option:selected").text();

	document.getElementById('city').setAttribute('value',cc+', '+rr+', '+ss );
document.getElementById('region2').setAttribute('value', $("#region").val());
document.getElementById('country2').setAttribute('value', $("#country").val());


document.getElementById('receiverCityId').setAttribute('value', "44");

}
	hipping = function (event) {							 console.log($("#city2").val());
	ss=$("#city2").val();
	cc= $("#country option:selected").text();
	rr= $("#region option:selected").text();
...
?>
	   ...
	   <p class="form-row form-row-wide">
			<input id="city" name = 'country'  readonly="readonly" onkeydown="if(event.keyCode==13){return false;}"  class = 'input-text' type = 'text'  placeholder = 'Введите город'  value = '<?php print @$_SESSION['country'];?>'/> <? //value = '<?php print @$_SESSION['country']; '?>
						<input id="region2" name = 'region2' onkeydown="if(event.keyCode==13){return false;}" value = '<?php print @$_SESSION['region2']; ?>' class = 'input-text' type = 'hidden' placeholder = 'Введите город77' />
			<input id="country2" name = 'country2' onkeydown="if(event.keyCode==13){return false;}" value = '<?php print @$_SESSION['country2']; ?>' class = 'input-text' type = 'hidden' placeholder = 'Введите город77' />
<script>					//		document.getElementById('city').setAttribute('value', $("#city2").val());
</script>
			
			<div style="display: none" id="loading_city"><img alt="" src="/ajax/img/ajax_loader.gif" />Загрузка...</div>
			<div>
...
<?php
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) : ?>
			...
			<p class="form-row form-row-wide">
				<input type="hidden" class="input-text" value="<?php //echo esc_attr( $woocommerce->customer->get_shipping_postcode() ); ?>" placeholder="<?php _e( 'Postcode / Zip', 'woocommerce' ); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
			</p>
