...
<div id="order_review2">
	...
	<table class="shop_table">
		<thead>
			<tr>
				...
				<th class="product-attr">№</th>
				
				<th class="product-name"><?php _e( 'Product', 'woocommerce' ); ?></th>
				<th class="product-attr"></th>
				<th class="product-attr">Размер</th>
				<th class="product-attr">Тип</th>
				<th class="product-attr">Кол-во</th>
				<th class="product-attr">Ед.</th>
		...
		<tfoot>
			<tr class="cart-subtotal">
				<th colspan="7"><?php //_e( 'Cart Subtotal', 'woocommerce' ); ?></th>
							...
							echo '
								<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">'.
									...
									'<td class="product-name">' .
										...
										apply_filters( 'woocommerce_checkout_item_quantity', ' ', $values, $cart_item_key ) .
									...
									'</td>'.									
									'<td class="product-attr">' .
										$_product->get_attribute('size') . 
									'</td>'.		
									'<td class="product-attr">' .
										$_product->get_attribute('type') . 
									'</td>'.		
									'<td class="product-attr">' .
										 $values['quantity'] .
									'</td>'.
									'<td class="product-attr">шт.</td>'.
	...
	<script>
		(function($) {
			...
			$(function() {
				$("#payment input[checked]").removeAttr("checked");
				$("#place_order").hide();
				
				$("#payment input[type=radio]").change(function() {
						$("#place_order").show();
					$(this).parents("ul").find(".payment_box").slideUp();
	...
	<div id="payment">
		...
		<input type="button" onclick="window.location = '/katalog/?cat_page=1&order_by=size&order=asc'" class="checkout-button button alt" style="float: left" name="proceed" value="&larr; Назад в каталог">
